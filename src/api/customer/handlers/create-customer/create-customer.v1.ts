...
export const createCustomerHandler = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (error?.code === "P2002" || (typeof error?.message === "string" && error.message.toLowerCase().includes("unique"))) {
            return res.status(HttpStatusCode.CONFLICT).json({ status: "error", message: "Customer with this email already exists" });
        }

        // Error log for internal server error
        return res.status(HttpStatusCode.INTERNAL_SERVER).json({ status: "error", message: "Failed to create customer" });
    }
};