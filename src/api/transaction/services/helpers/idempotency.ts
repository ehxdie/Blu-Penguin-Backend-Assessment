...
export async function getIdempotencyValue(idempotencyKey: string): Promise<any | null> {
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        `idempotency:get - failed to parse JSON for key=${idempotencyKey}, returning raw string`,
        err,
      );
      return raw;
    }
    ...
    logger.error(
      `idempotency:get - error reading idempotency key from redis: ${idempotencyKey}`,
      error,
    );
    throw error;
  }
...
export async function setIdempotencyValue(idempotencyKey: string, value: any) {
  ...
  try {
    ...
    logger.info(
      `idempotency:set - stored key=${idempotencyKey} payloadBytes=${Buffer.byteLength(payload, 'utf8')}`,
    );
  } catch (error) {
    logger.error(
      `idempotency:set - error writing idempotency key to redis: ${idempotencyKey}`,
      error,
    );
    throw error;
  }
